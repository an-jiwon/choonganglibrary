...
<div class="table-responsive">
 ...
 <thead>
	...
	<c:if test="${empty list }">
			<tr>
				<td colspan="3">예약기록이 없습니다</td>
			</tr>
		</c:if>
		<c:if test="${not empty list }">
			<c:forEach var="reserveResult" items="${list}">
				<tr>
					<td>${reserveResult.title}</td>
...
<c:if test="${startPage > PAGEPERBLOCK }">
		<a href="adminMemberReserve.do?id=${id }&pageNum=${endPage-PAGEPERBLOCK}">[이전]</a>
</c:if>
<c:forEach var="i" begin="${startPage}" end="${endPage}">
		<a href="adminMemberReserve.do?id=${id }&pageNum=${i}" >[${i}]</a>
</c:forEach>
<c:if test="${endPage < totPage }">
		<a href="adminMemberReserve.do?id=${id }&pageNum=${startPage+PAGEPERBLOCK}">[다음]</a>
</c:if>	
</div>
